...
class TestConditionEvaluator:
    ...
    def test_success_ignores_continue_on_fail_steps(self):
        """Test success() ignores failed steps with continue_on_fail=True."""
        steps = {
            "step1": StepResult(id="step1", name="Step 1", status=Status.PASS, duration=1.0),
            "step2": StepResult(id="step2", name="Step 2", status=Status.FAIL, duration=1.0, continue_on_fail=True),
        }
        evaluator = ConditionEvaluator(steps)
        # step2 failed but has continue_on_fail, so success() should still be True
        assert evaluator.evaluate("success()") is True

    def test_failure_ignores_continue_on_fail_steps(self):
        """Test failure() ignores failed steps with continue_on_fail=True."""
        steps = {
            "step1": StepResult(id="step1", name="Step 1", status=Status.PASS, duration=1.0),
            "step2": StepResult(id="step2", name="Step 2", status=Status.FAIL, duration=1.0, continue_on_fail=True),
        }
        evaluator = ConditionEvaluator(steps)
        # step2 failed but has continue_on_fail, so last_step_status is PASS (from step1)
        assert evaluator.evaluate("failure()") is False

    def test_continue_on_fail_step_status_still_queryable(self):
        """Test that individual step status is still queryable even with continue_on_fail."""
        steps = {
            "step1": StepResult(id="step1", name="Step 1", status=Status.PASS, duration=1.0),
            "step2": StepResult(id="step2", name="Step 2", status=Status.FAIL, duration=1.0, continue_on_fail=True),
        }
    ...
    def test_success_fails_with_non_continue_on_fail_step(self):
        """Test success() still returns False for failed steps without continue_on_fail."""
        steps = {
            "step1": StepResult(id="step1", name="Step 1", status=Status.PASS, duration=1.0),
            "step2": StepResult(id="step2", name="Step 2", status=Status.FAIL, duration=1.0, continue_on_fail=False),
        }
        evaluator = ConditionEvaluator(steps)
        assert evaluator.evaluate("success()") is False
