    ...
    def check_operation_state_every_five_seconds(self, operation_id: str):
        ...
        if arrow.now() > three_minutes_from_now:
            ...
            if response.status_code == 200:
                r = response.json()
                log_info("Operation State is {operation_state}".format(operation_state=r["state"]))
                log_info("Sleeping 5 seconds..")
                time.sleep(5)
            else:
                log_info("Error in getting operation state...")
                log_info(response.json())
        else:
            log_info("Three minute querying finished...")
