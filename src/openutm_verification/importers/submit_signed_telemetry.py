...
class FlightBlenderUploader:
    def upload_to_server(self, filename):
        ...
        uas_id = openutm_verification.rid.UASID(
            registration_id="CHE-5bisi9bpsiesw", serial_number="d29dbf50-f411-4488-a6f1-cf2ae4d4237a", utm_id="07a06bba-5092-48e4-8253-7a523f885bfe"
        )
        eu_classification = openutm_verification.rid.UAClassificationEU(category="Open", class_="Class0")

        rid_operator_details = openutm_verification.rid.RIDOperatorDetails(
            ...
            operator_location=openutm_verification.rid.LatLngPoint(lat=46.97615311620088, lng=7.476099729537965),
        )
        ...
        for state in states:
            ...
            try:
                ...
                signed_r = requests.Request(
                    method="PUT", url="http://localhost:8000/flight_stream/set_signed_telemetry", json=payload, headers=headers
                )
                ...
                print("Error in signing message to be sent to ArgonServer {signing_error}".format(signing_error=e))
                break
            else:
