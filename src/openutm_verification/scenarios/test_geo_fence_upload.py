...
async def test_geo_fence_upload(fb_client: FlightBlenderClient) -> None:
    """Upload a geo-fence (Area of Interest) and then delete it (teardown)."""
    await fb_client.upload_geo_fence(filename=get_geo_fence_path("geo_fence.geojson"))
    await fb_client.get_geo_fence()
