...
async def test_f5_non_conforming_contingent_path(fb_client: FlightBlenderClient) -> None:
    async with fb_client.create_flight_declaration():
        ...
        await fb_client.check_operation_state_connected(expected_state=OperationState.NONCONFORMING, duration=5)
        await fb_client.update_operation_state(state=OperationState.CONTINGENT)
