...
MAX_AUTH_LENGTH = DRIP_AUTH_PAGE_ZERO_DATA_SIZE + DRIP_AUTH_PAGE_NONZERO_DATA_SIZE * (DRIP_AUTH_MAX_PAGES - 1)
...
class DRIP_BasicID_data(Structure):
    _fields_ = [("UAType", DRIP_uatype_t), ("IDType", DRIP_idtype_t), ("UASID", c_char * (DRIP_ID_SIZE + 1))]
...
class DRIP_Auth_encoded(ctypes.Union):
    _fields_ = [("page_zero", DRIP_Auth_encoded_page_zero), ("page_non_zero", DRIP_Auth_encoded_page_non_zero)]
...
class DRIP_OperatorID_data(Structure):
    _fields_ = [("OperatorIdType", DRIP_operatorIdType_t), ("OperatorId", c_char * (DRIP_ID_SIZE + 1))]
